...
def main(args):
    ...
    print(f"Knowledge graph dir: {knowledge_graph_dir}")
    print("Intialized searcher")
    searcher = Searcher(
    ...
    if os.path.exists(responses_path):
        ...
        with open(responses_path, "rb") as f:
            ...
            print("Loaded responses from: ", responses_path)
    else:
        print("Start searching for combined keywords.")
    ...
    #     )
    # with open(
    #     combined_keywords_path,
    #     "wb",
    # ) as f:
    #     pickle.dump({"responses": responses, "keywords": keywords}, f)
    #     print("Saved combined version of keywords to: ", combined_keywords_path)

    print("Searching the separated keywords.")
    ### FOR EACH LESION
    separated_keywords = OrderedDict({})
    for lesion in TOP_5_LESSIONS:
        # 'atelectasis' go wrong.
        ...
        # Save each lesion's keywords separately
        lesion_keyword_path = os.path.join(
            args.output_dir,
            args.store_type,
            output_folder_name,
            f"{lesion}_separate_extracted_keywords.pkl",
        )
        with open(
            lesion_keyword_path,
            "wb",
        ) as f:
            pickle.dump({lesion: lesion_keywords}, f)
            print(f"Saved {lesion} keywords to: ", lesion_keyword_path)

    print("Saving separated version of keywords.")
    sep_keyword_path = os.path.join(
            args.output_dir,
            args.store_type,
            output_folder_name,
    ...
    with open(
        ...
        print("Saved separated version of keywords to: ", sep_keyword_path)
